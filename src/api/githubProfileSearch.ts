  ...
  async search(username: string): Promise<GitHubProfile | undefined> {
    try {
      const user = await api.get<User>(
        `${this.baseUsersEndpoint()}${username}`
      );
      ...
      const repos = await api.get<Repository[]>(
        `${this.baseUsersEndpoint()}${username}/repos?sort=created`
      );
      ...
      if (repos) {
        return {
          ...
          repositories: repos.slice(0, 4),
        };
      }
    } catch (_) {
      alert(
        `An error occurred while searching for the GitHub user ${username}. Please try again later.`
      );
    }
  }
}
